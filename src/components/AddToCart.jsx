...
const AddToCart = ({ game }) => {
  ...
  return (
    <div className="w-full lg:w-[30%] border rounded-md shadow-md flex-col items-center justify-center overflow-hidden">
      <p className="select-none text-center mb-2 text-black border-b-2 text-lg py-3">
        Precio
      </p>

      <div className="select-none">
        <p className="text-2xl font-bold mt-5 text-center">
          $ {separadorMiles(game.precio)}
        </p>
      </div>
      ...
      <div className="select-none flex items-center justify-center mt-2 gap-7">
        <div
          onClick={decrement}
          className="h-9 w-9 flex items-center justify-center rounded-full bg-black text-white shadow-sm hover:scale-105 hover:shadow-lg hover:cursor-pointer ease-out duration-300 "
        >
      ...
      {cantidad > 0 ? (
        <div
          ...
          className="select-none flex items-center justify-center gap-4 bg-black py-3 text-white mt-2 hover:bg-black/80 hover:cursor-pointer ease-out duration-300"
        >
          <BsCartPlusFill size={20} className="text-white" />
          {cartLoading ? <Spinner /> : <p>Añadir al carrito</p>}
        </div>
      ) : null}
